...
class MainWindow(QMainWindow):
    ...
    def _create_form(self):
        ...
        self._phone_edit.setInputMask("(000) 999-9999;_")
        ...
        self._email_edit.setValidator(QRegularExpressionValidator(email_regex))
        self._email_edit.textChanged.connect(self._email_edit_text_changed)
    ...
    def _email_edit_text_changed(self):
        if self._email_edit.hasAcceptableInput():
            self._email_edit.setStyleSheet("color: white; ")
        else:
            self._email_edit.setStyleSheet("color: red; ")
    ...
    def _save_button_clicked(self):
        ...
        if not self._phone_edit.hasAcceptableInput():
            QMessageBox.warning(
                self,
                "Invalid phone number",
                "The phone number you entered does not meet format requirements",
            )
            return

        if not self._email_edit.hasAcceptableInput():
            QMessageBox.warning(
                self,
                "Invalid email address",
                "The email address you entered does not follow email address format rules.",
            )
