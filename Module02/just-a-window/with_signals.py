...
def create_name_and_disposition_layout():
    editable = QCheckBox("Editable")
    editable.setChecked(True)

    name = QLineEdit()
    name.setPlaceholderText("Enter your cat's name")

    editable.stateChanged.connect(name.setEnabled)

    disposition = QComboBox()
    disposition.addItems(["Friendly", "Shy", "Aggressive"])
    disposition.currentTextChanged.connect(disposition_changed)

    editable.stateChanged.connect(disposition.setEnabled)

    form = QFormLayout()
    form.addRow("Editable", editable)
...
def create_button_layout():
    button1 = QPushButton("Say Hello on console")
    button1.clicked.connect(say_hello)
    button2 = QPushButton("Say hello in dialog")
    button2.clicked.connect(say_hello_dialog)
    button3 = QPushButton("Button 3")
    button4 = QPushButton("Quit")
    button4.clicked.connect(quit)
...
@Slot()
def say_hello_dialog():
    
    dialog = QMessageBox(
        QMessageBox.Icon.Critical,
        "Greeting",
        "Hello from QMessageBox!",
...
@Slot()
def disposition_changed(new_disposition):
    print(f"Disposition changed to: {new_disposition}")
